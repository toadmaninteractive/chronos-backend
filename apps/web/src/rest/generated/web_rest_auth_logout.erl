...
logout_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

logout_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_put(Req) ->
    try
        {ResponseContent, Req1} = web_rest_callback_auth:logout(Req),
        ...
        #{status_code := 403, response := ResponseContent403} ->
            ResponseContent403Body = jsx:encode(data_protocol:forbidden_error_ex_to_json(ResponseContent403, {custom, fun web_protocol:logout_error_to_json/1})),
